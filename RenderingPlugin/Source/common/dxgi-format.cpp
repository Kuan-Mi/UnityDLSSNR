// Format mapping table ported from NVRHI (External/nvrhi/src/common/dxgi-format.cpp).
// Copyright (c) 2014-2021, NVIDIA CORPORATION. MIT license (see ThirdParty/NVRHI-LICENSE.txt).
...
namespace unityrhi
{
// The rows must be in exactly the same order as Format enum members are defined.
static const DxgiFormatMapping c_FormatMappings[] = {
    ...
    { Format::A8_UNORM,             DXGI_FORMAT_A8_UNORM,               DXGI_FORMAT_A8_UNORM,                 DXGI_FORMAT_A8_UNORM               },
};
...
const DxgiFormatMapping& getDxgiFormatMapping(Format format)
{
    ...
    assert(mapping.abstractFormat == format);
    return mapping;
}
} // namespace unityrhi
