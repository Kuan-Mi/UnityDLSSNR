// StaticDescriptorHeap ported from NVRHI's d3d12-descriptor-heap.cpp
...
HRESULT StaticDescriptorHeap::Grow(uint32_t minRequiredSize)
{
    ...
    // Retire the outgoing heaps instead of releasing them: command lists that
    // were recorded against them may not have executed yet.
    if (m_Heap)
        m_RetiredHeaps.push_back(m_Heap);
    if (m_ShaderVisibleHeap)
        m_RetiredHeaps.push_back(m_ShaderVisibleHeap);
...
D3D12_CPU_DESCRIPTOR_HANDLE StaticDescriptorHeap::getCpuHandle(DescriptorIndex index) const
{
    D3D12_CPU_DESCRIPTOR_HANDLE handle = m_StartCpuHandle;
    handle.ptr += SIZE_T(index) * m_Stride;
    return handle;
}

D3D12_CPU_DESCRIPTOR_HANDLE StaticDescriptorHeap::getCpuHandleShaderVisible(DescriptorIndex index) const
{
...
D3D12_GPU_DESCRIPTOR_HANDLE StaticDescriptorHeap::getGpuHandle(DescriptorIndex index) const
{
    D3D12_GPU_DESCRIPTOR_HANDLE handle = m_StartGpuHandleShaderVisible;
    handle.ptr += UINT64(index) * m_Stride;
    return handle;
}
